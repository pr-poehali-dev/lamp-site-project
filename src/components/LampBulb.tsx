...
interface LampBulbProps {
  isOn: boolean;
  toggle?: () => void; // Добавляем функцию переключения как опциональный проп
}

const LampBulb: React.FC<LampBulbProps> = ({ isOn, toggle }) => {
  return (
    <svg
      width="180"
      height="250"
      viewBox="0 0 180 250"
      className={`transition-all duration-300 ${isOn ? "bulb-on" : "opacity-75"} ${toggle ? "cursor-pointer hover:scale-105" : ""}`}
      xmlns="http://www.w3.org/2000/svg"
      onClick={toggle} // Добавляем обработчик клика
    >
      {/* Лампочка */}
      <path
